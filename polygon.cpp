...
public:
  ...
  void drawpoly1() {
    ...
    a[n][0] = a[0][0]; // last point should be connceted to first point to make
                       // a close fig
    a[n][1] = a[0][1];
  ...
  void fillpoly1() {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (dx == 0)
        slope[i] = 0.0;
      if (dy != 0 &&
          dx != 0) // calculate inverse slope if polygon is in 4th quadradent.
      {
    ...
    for (y = 0; y < 480; y++) // for reverse filling(y=480;y>0;y--)
    {
      ...
      for (i = 0; i < n; i++) {
        if (((a[i][1] <= y) && (a[i + 1][1] > y)) ||
            ((a[i][1] > y) &&
             (a[i + 1][1] <= y))) // check ymin or ymax is less or greater
        {
          xi[k] = (int)(a[i][0]) +
                  slope[i] * (y - a[i][1]); // calculate intersection
          k++;
        }
      }
      for (j = 0; j < k - 1; j++) {
        ...
        for (i = 0; i < k; i += 2) {
          line(xi[i], y, xi[i + 1],
               y); // draw the line using intersection points only
          delay(20);
        }
      }
    }
